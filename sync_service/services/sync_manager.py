...
import logging
import traceback
import httpx # <--- IMPORT AJOUTÉ
from datetime import datetime, timezone, timedelta
from sqlalchemy.orm import Session
from typing import Dict, List, Any, Optional, Tuple
from fastapi import HTTPException, status # <--- IMPORTS AJOUTÉS
...
from . import transaction_sync # Utilisation de l'import relatif

# Import Vector Storage Service
# Utilisation d'un try-except pour la flexibilité si le service n'est pas déployé/disponible
try:
    from .vector_storage import VectorStorageService
    VECTOR_STORAGE_AVAILABLE = True
    logger_vs = logging.getLogger(__name__) # Utiliser le logger standard
    logger_vs.info("VectorStorageService importé avec succès.")
except ImportError as e:
    VECTOR_STORAGE_AVAILABLE = False
    # Définir une classe factice pour éviter les erreurs AttributeError
    class VectorStorageService:
        async def batch_store_accounts(self, *args, **kwargs): logger_vs.warning("VectorStorage non dispo: batch_store_accounts ignoré."); return {"status": "skipped"}
        async def batch_store_categories(self, *args, **kwargs): logger_vs.warning("VectorStorage non dispo: batch_store_categories ignoré."); return {"status": "skipped"}
        async def batch_store_insights(self, *args, **kwargs): logger_vs.warning("VectorStorage non dispo: batch_store_insights ignoré."); return {"status": "skipped"}
        async def batch_store_stocks(self, *args, **kwargs): logger_vs.warning("VectorStorage non dispo: batch_store_stocks ignoré."); return {"status": "skipped"}
    ...
    logger_vs.warning(f"VectorStorageService non trouvé ({e}). Les fonctionnalités de stockage vectoriel seront désactivées.")


# Imports Utilities
from ..utils.logging import get_contextual_logger # Utilisation de l'import relatif
from user_service.core.config import settings

# Configuration du logger principal pour ce module
logger = logging.getLogger(__name__)

# Liste des statuts qui requièrent une action utilisateur
ACTION_REQUIRED_STATUSES = [402, 429, 1010] # Compléter si nécessaire

# --- Fonctions d'aide ---
# Helper pour parser les dates (peut être mis dans un fichier utils partagé)
def parse_iso_date(date_str: Optional[str]) -> Optional[datetime]:
    ...
    try:
        ...
        # Essayer de parser avec différentes précisions si nécessaire
        # Python 3.7+ supporte fromisoformat directement pour UTC ('Z' ou +00:00)
        return datetime.fromisoformat(date_str)
    except ValueError:
         # Tenter d'autres formats si nécessaire ou logguer l'erreur
...
async def create_or_update_sync_item(db: Session, user_id: int, bridge_item_id: int, item_data: Dict[str, Any]) -> SyncItem:
    ...
    try:
        ...
        # Utilisation de la fonction helper parse_iso_date définie ci-dessus
        ...
        if sync_item:
            ...
            sync_item.account_types = item_data.get("account_types", sync_item.account_types) # Assumer que c'est une string simple
        else:
        ...
        # Récupérer les comptes associés pour la base SQL (important pour les relations)
        try:
...
async def update_item_status(db: Session, sync_item: SyncItem, status_code: int, status_code_info: Optional[str] = None, status_description: Optional[str] = None) -> SyncItem:
    ...
    # Renommer 'status' en 'status_code' pour éviter confusion avec module 'status' de FastAPI
    ...
    try:
        ...
        if status_code_info: sync_item.status_code_info = status_code_info
        if status_description: sync_item.status_description = status_description # Ajouter la description
...
async def get_bridge_item_info(db: Session, sync_item: SyncItem, access_token: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            ctx_logger.debug(f"Réponse API Bridge pour item info: {item_data}")
            return item_data
...
async def fetch_and_update_sql_accounts(db: Session, sync_item: SyncItem, access_token: str) -> Tuple[List[SyncAccount], List[Dict[str, Any]]]:
    ...
    try:
        # Utiliser la fonction get_bridge_accounts qui gère la pagination
        bridge_accounts_data = await bridge_service.get_bridge_accounts(db, sync_item.user_id, item_id=sync_item.bridge_item_id)
        ...
        for account_data in bridge_accounts_data:
            ...
            if bridge_account_id is None:
                ctx_logger.warning(f"Donnée de compte sans ID reçue de Bridge: {account_data}")
                continue
            ...
            if sync_account:
                ...
                needs_update = False
                if sync_account.account_name != account_name: sync_account.account_name = account_name; needs_update = True
                if sync_account.account_type != account_type: sync_account.account_type = account_type; needs_update = True
        ...
        for acc in sql_accounts_updated:
            try:
                db.refresh(acc)
                 ...
                 ctx_logger.warning(f"Impossible de rafraîchir le compte SQL id={acc.id}, potentiellement supprimé.", exc_info=False)
...
# --- Fonction Principale de Synchronisation (Modifiée) ---
    ...
    try:
        ...
        if item_info:
            sync_item = await update_item_status(
                db, sync_item,
                item_info.get("status", sync_item.status), # Renommé en status_code dans la fonction
        ...
        # 6. Récupérer et Stocker les Catégories (Globales ou par utilisateur)
        if vector_storage:
            ctx_logger.info("Étape 6: Récupération et stockage vectoriel des catégories")
            try:
                # On passe user_id pour l'authentification, mais les catégories sont probablement globales
                categories_list = await bridge_service.get_bridge_categories(db, user_id)
                if categories_list:
                    # Les catégories retournées sont déjà "plates" grâce à la fonction bridge
        ...
        if vector_storage:
            ...
            try:
                insights_data = await bridge_service.get_bridge_insights(db, user_id)
                if insights_data:
                    insights_to_store = []
                    ...
                    for month_data in monthly_kpis:
                        ...
                        # Utiliser parse_iso_date pour convertir les strings en datetime
                        ...
                        for cat_insight in month_data.get("categories", []):
                             insights_to_store.append({
                                 ...
                                 # Récupérer le nom de catégorie ici si possible (nécessite lookup) ou stocker l'ID seulement
                                 "period_type": "monthly",
        ...
        if vector_storage:
            ...
            # Identifier les types de comptes d'investissement (à adapter selon Bridge)
            ...
            if investment_account_ids:
                ...
                for acc_id in investment_account_ids:
                    try:
                        stocks_list = await bridge_service.get_bridge_stocks(db, user_id, account_id=acc_id)
                        if stocks_list:
                            for stock in stocks_list:
                                all_stocks_to_store.append({
                                    **stock,
                                    "user_id": user_id,
                                    "account_id": acc_id,
                                    "bridge_stock_id": stock.get("id"), # Utiliser l'ID de l'action Bridge
                                    "bridge_updated_at": stock.get("updated_at") # Garder la date Bridge
                                })
                        else:
                            # Pas nécessairement une erreur si un compte n'a pas d'actions
                ...
                if all_stocks_to_store:
                    try:
                        ...
                        stocks_results.append(vector_result) # Ajouter le résultat du batch
                        if vector_result.get("status") != "success": overall_status = "partial"
        ...
        for account in sql_accounts:
            ...
            try:
                # transaction_sync gère son propre stockage vectoriel
                ...
                total_new_transactions += account_result.get("new_transactions", 0) # Utiliser la clé correcte
        ...
        sync_report["steps"]["sync_transactions"] = {
            ...
            "total_new_transactions_stored": total_new_transactions, # Nom de clé clarifié
            # "details": transaction_sync_results # Peut être trop volumineux, à exclure ou tronquer
        }
...
async def get_user_sync_status(db: Session, user_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        # Récupérer les comptes via la relation pour éviter une requête séparée si possible
        ...
        for item in sync_items:
            if item.needs_user_action: needs_action = True
            ...
            items_info.append({
                ...
                "sql_account_count": len(item.accounts) # Nombre de comptes SQL liés
            })
        ...
        status_response = {
            ...
            "sql_status": {
                ...
                "days_since_last_sync": (datetime.now(timezone.utc) - last_sync_sql).days if last_sync_sql else None,
...
async def create_reconnect_session(db: Session, user_id: int, bridge_item_id: int) -> str:
    ...
    try:
        ...
        base_url = settings.WEBHOOK_BASE_URL # Assurez-vous que cela pointe vers votre frontend/backend
        # L'URL de callback doit être gérée par votre application (frontend ou backend)
        callback_url = f"{base_url}/reconnection/callback?item_id={bridge_item_id}" # Exemple
...
# Fonctions précédentes à réévaluer/supprimer si non utilisées
# async def check_stale_items(db: Session) -> List[Dict[str, Any]]: ...
# async def check_vector_storage_status(db: Session, user_id: int) -> Dict[str, Any]: ...
# async def reconcile_vector_storage(db: Session, user_id: int) -> Dict[str, Any]: ...
# async def get_vector_storage_metadata(db: Session, user_id: int) -> Dict[str, Any]: ...